...
// NEW: Parallax effect on hero cube (moves with mouse)
...
// NEW: Random bubble generator
function initBubbleGenerator() {
    const bubbles = document.querySelector('.bubbles');
    if (!bubbles) return;

    const phrases = ['🔥', '⚡', '💯', '🚀', '✨', 'no code', 'instant', 'love it', 'PDF sucks', '15 min', '4.9★'];
    ...
    setInterval(() => {
        ...
        bubble.style.setProperty('--y', Math.random() * 100 + '%');
        bubble.style.setProperty('--delay', Math.random() * 5 + 's');

        const content = document.createElement('div');
        content.className = 'bubble-content';
        content.textContent = phrases[Math.floor(Math.random() * phrases.length)];
